...
function certPlacedFields() {
  return overlay.getElements().filter((c) => c.model.type === 'text' && c.model.fieldKey && String(c.model.text || '').trim());
}
  ...
  const render = () => {
    ...
    if (step === 1) {
      ...
      body = '<div class="gd-q">רשימת הסטודנטים</div>' +
        ...
        '<div class="gd-foot"><button type="button" class="btn primary" id="certNext" ' + (certList && Object.values(certMap).some(Boolean) ? '' : 'disabled') + '>המשך להפקה ➜</button></div>';
    } else {
      const n = certList ? certList.records.length : 0;
      body = '<div class="gd-q">הפקת ' + n + ' תעודות</div>' +
        '<div class="hint" style="margin:-2px 0 10px">כל תעודה תקבל את הנתונים של הסטודנט שלה, ממורכזת בדיוק במקום שסימנת. הכול נשאר במחשב שלך.</div>' +
        '<div class="gd-chips"><button type="button" class="gd-chip" id="certZip">🎓 ZIP — קובץ לכל סטודנט</button>' +
        '<button type="button" class="gd-chip" id="certOne">🖨️ PDF אחד להדפסה</button></div>' +
        '<div class="hint" id="certProg" style="min-height:16px"></div>' +
        '<div class="gd-foot"><button type="button" class="btn ghost sm" id="certBack2">‹ חזרה</button></div>';
    }
    certCard.innerHTML =
    ...
    if (step === 1) {
      ...
      CERT_FIELDS.forEach((f) => {
        ...
        b.addEventListener('click', () => {
          PFS.toast('לחצו על התעודה במקום שבו "' + f.label + '" צריך להופיע', 'ok', 4000);
          overlay.setPlacing({ create: (pageIndex, fx, fy) => { certPlace(pageIndex, fx, fy, f.key); render(); return null; } });
        });
        chips.appendChild(b);
      });
      ...
      other.addEventListener('click', async () => {
        ...
        overlay.setPlacing({ create: (pageIndex, fx, fy) => { certPlace(pageIndex, fx, fy, String(name).trim()); render(); return null; } });
      });
...
function certLoadList(parsed, fileName) {
  ...
  certList = parsed;
  const keys = [...new Set(certPlacedFields().map((c) => c.model.fieldKey))];
...
async function certProduce(kind, opts) {
  ...
  const placed = certPlacedFields();
  const keys = [...new Set(placed.map((c) => c.model.fieldKey))];
  const mapping = {};   // header → fieldKey (the merge engine's shape)
  keys.forEach((k) => { if (certMap[k]) mapping[certMap[k]] = k; });
  const nameHeader = certMap['שם מלא'] || certMap[keys[0]] || null;
  const records = PFS.merge.remapRecords(certList.records, mapping).map((r, i) => {
    r.__name = nameHeader ? certList.records[i][nameHeader] : '';
    return r;
  });
