  ...
  const render = () => {
    ...
    if (step === 1) {
      ...
      body = '<div class="gd-q">' + (nOn ? nOn + ' שדות כבר על התעודה 👇' : 'מה מופיע על התעודה?') + '</div>' +
        ...
        '<div class="gd-chips" style="margin:8px 0 4px"><button type="button" class="gd-chip" id="certAddOther">+ שדה אחר…</button></div>' +
        '<div class="gd-foot">' +
...
function certBuildRecords() {
  ...
  const records = certList.records.map((raw, i) => Object.assign({}, raw, mapped[i], { __name: nameHeader ? raw[nameHeader] : '' }));
