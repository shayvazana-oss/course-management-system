...
async function openPdfFile(file) {
  ...
  try {
    ...
    PFS.recent && PFS.recent.save(file.name, buf.slice(0)).then(() => renderRecent());
...
async function renderRecent(filter) {
  ...
  shown.forEach((d) => {
    ...
    del.addEventListener('click', async (e) => { e.stopPropagation(); await PFS.recent.remove(d.id); renderRecent(filter); });
