...
function certPreflightNow() {
  ...
  return PFS.merge.preflight(records, { nameKey: nameHeader || '__name', idKey: idHeader });
}
...
async function certProduce(kind, opts) {
  ...
  try {
    ...
    if (prog) prog.textContent = '✓ הופקו ' + result.count + ' תעודות';
