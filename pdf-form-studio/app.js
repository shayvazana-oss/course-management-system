...
function certPlaceSentence(pageIndex, fx, fy) {
  ...
  const ctrl = overlay.addModelAt('text', pageIndex, {
    ...
    text: CERT_SENTENCE, wrapW: maxW, noEdit: true
  });
  if (!ctrl) return null;
  ctrl.layout();
  overlay.deselectAll();
...
function certPlace(pageIndex, fx, fy, key) {
  ...
  const ctrl = overlay.addModelAt('text', pageIndex, {
    ...
    text: spec.sample, fieldKey: spec.key, noEdit: true
  });
  if (!ctrl) return null;
...
function showCertWizard() {
  ...
  const render = () => {
    ...
    if (step === 1) {
      const chips = $c('certChips');
      CERT_FIELDS.forEach((f) => {
        const b = document.createElement('button'); b.type = 'button'; b.className = 'gd-chip';
        const done = keys.includes(f.key);
        b.textContent = (done ? '✓ ' : '📍 ') + f.label;
        b.addEventListener('click', () => armCertPlacement(f.key, f.label));
        chips.appendChild(b);
      });
      const sent = document.createElement('button'); sent.type = 'button'; sent.className = 'gd-chip'; sent.textContent = '📝 משפט עם משתנים';
      sent.title = 'משפט שלם כמו "מוענקת בזאת ל־{שם מלא} על סיום קורס {שם הקורס}" — המשתנים מתמלאים לכל סטודנט';
      sent.addEventListener('click', () => {
        if (certCard) certCard.style.display = 'none';
        document.querySelectorAll('.cert-arm').forEach((b) => b.remove());
        const arm = document.createElement('div'); arm.className = 'cert-arm';
        arm.innerHTML = '<span>📝 לחצו על התעודה במקום שבו המשפט צריך להופיע</span><button type="button">ביטול (Esc)</button>';
        document.body.appendChild(arm);
        const restore = () => { arm.remove(); if (certCard) { certCard.style.display = ''; certCard.__render && certCard.__render(); } document.removeEventListener('keydown', onEsc, true); };
        const onEsc = (e) => { if (e.key === 'Escape') { overlay.setPlacing(null); restore(); } };
        arm.querySelector('button').addEventListener('click', () => { overlay.setPlacing(null); restore(); });
        document.addEventListener('keydown', onEsc, true);
        overlay.setPlacing({ create: (pageIndex, fx, fy) => { certPlaceSentence(pageIndex, fx, fy); restore(); return null; } });
      });
      chips.appendChild(sent);
      const other = document.createElement('button'); other.type = 'button'; other.className = 'gd-chip'; other.textContent = '+ שדה אחר…';
