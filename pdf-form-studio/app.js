...
const overlay = PFS.createOverlayManager({
  ...
  // a tap on empty paper opens a caret right there ("לוחצים על המשבצת ומקלידים")
  onEmptyTap: (page, fx, fy) => { try { quickTextAt(page, fx, fy); } catch (e) { console.warn('quick text failed', e); } },
  // a certificate field dropped near a printed underline settles onto it
  onDragEnd: (c) => { try { if (certMode && typeof certSnapToInk === 'function') certSnapToInk(c); } catch (e) {} },
  ...
  // click-to-fill: tapping an orange marker on the FORM opens a caret IN that
  // field, right on the page (a tick box still jumps to its panel row)
  onMarkerClick: (f) => {
    if (f && f.type === 'text') {
      const c = quickTextAt(f.page, f.fx + (f.fw || 0) / 2, f.fy + (f.fh || 0.02) / 2);
      if (c) return;
    }
    activateTab('fill');
...
const templates = PFS.createTemplates({
  ...
  applyModels: (models) => { overlay.clearElements(); fieldsPanel.clear(); overlay.applyModels(models); liftSmallFills('template'); },
...
function vaultPrefill(det) {
  try {
    ...
    // a certificate format is filled from the STUDENT LIST, never from the
    // clerk's own profile — a printed "שם:" blank must not get the clerk's name
    if (certMode || certOpening) return null;
...
// Filled-in values are HANDWRITING, not print: a person filling a form writes
// at their own comfortable size even when the form is set in 7pt. Deriving the
...
/* liftSmallFills() — a saved template / auto-memory replays its elements
 * EXACTLY as stored, and layouts remembered before the readable floor existed
 * (or shrunk by an older fitter) carry fills of 6-9pt. They came back on the
 * page tiny and the clerk had to enlarge every one by hand ("כשאני ממלא
 * בתבניות המוכנות מראש הוא מדביק את זה בפונט ממש קטן"). Values below the
 * fitter's own floor (85% of the readable size) are raised to the document's
 * handwriting size — never shrunk — with the same dense-row cap detection
 * applies, so a lifted table row never piles onto the row beneath it. The
 * right edge of a Hebrew value stays planted. Idempotent and cheap: runs
 * after every template apply and again once detection knows the form. */
function liftSmallFills(reason) {
  const els = overlay.getElements().filter(isPlainTextEl);
  if (!els.length) return 0;
  const uni = Math.max(MIN_FILL_FRAC, (lastDet && lastDet.uniFontFrac) || 0);
  const floor = MIN_FILL_FRAC * 0.85 - 1e-6;
  const fieldFF = {};
  if (lastDet && lastDet.fields) lastDet.fields.forEach((f) => { if (f.type === 'text' && f.fieldKey && f.fontFrac) fieldFF[f.fieldKey] = f.fontFrac; });
  let changed = 0;
  els.forEach((c) => {
    const m = c.model;
    if ((m.fontFrac || 0) >= floor) return;
    let cap = uni;
    // dense rows: the nearest value above OR below in the same column bounds
    // the size (both directions — a lifted last row must not climb onto the
    // row above it either)
    let pitch = Infinity;
    els.forEach((o) => {
      if (o === c || o.model.page !== m.page) return;
      const dy = Math.abs(o.model.fy - m.fy);
      if (dy > 0.004 && dy < pitch && o.model.fx < m.fx + (m.fw || 0) && o.model.fx + (o.model.fw || 0) > m.fx) pitch = dy;
    });
    if (pitch < 0.028) cap = Math.min(cap, Math.max(0.0095, pitch * 0.8));
    // the detected field's own (already pitch-capped) size is a hard ceiling
    if (m.fieldKey && fieldFF[m.fieldKey]) cap = Math.min(cap, Math.max(fieldFF[m.fieldKey], m.fontFrac || 0));
    if (cap <= (m.fontFrac || 0) + 0.0003) return;
    const oldW = m.fw, oldWrap = m.wrapW;
    m.fontFrac = cap;
    c.layout();
    if (m.align === 'right' && isFinite(oldW) && !m.wrapW && !oldWrap) { m.fx += (oldW - m.fw); c.layout(); }
    changed++;
  });
  if (changed) { console.info('[fills] lifted', changed, 'small values to readable size (' + (reason || '') + ')'); markDirty(); }
  return changed;
}
PFS.liftSmallFills = liftSmallFills;

/* quickTextAt(page, fx, fy) — "לוחצים על המשבצת ומקלידים": a tap on empty
 * paper opens a caret right there, no tool to pick first. What the tap lands
 * on decides the box:
 *   - inside a detected field's band → THAT field (panel-synced, remembered,
 *     cell-walled) — the same element the panel would create
 *   - otherwise a free box at the readable size that reads the printed page:
 *     a ruled line just below → the text sits on it; vertical borders either
 *     side → the cell is a hard wall (wraps inside, never spills)
 * An empty box vanishes on blur, so a stray tap costs nothing. */
function inkCellAt(page, fx, fy, fontFrac) {
  const out = { lineY: null, left: null, right: null };
  try {
    const views = pdfView.viewList ? pdfView.viewList() : [];
    const canvas = views[page] && views[page].canvas;
    ...
    const fhPx = Math.max(6, Math.round(fontFrac * 1.15 * H));
    // a ruled line under the tap: dark across a window around the tap
    ...
    const y0 = Math.max(0, cy - Math.round(fhPx * 0.4)), rows = Math.min(H - y0, Math.round(fhPx * 0.8));
    // a 1pt table border renders as 2 anti-aliased grey columns — judge each
    // row by the darkest of three neighbouring pixels, with a softer threshold
    const darkCol = (px) => {
      const x = Math.max(1, Math.min(W - 2, px));
      const col = ctx.getImageData(x - 1, y0, 3, rows).data; let d = 0;
      for (let i = 0; i < rows; i++) { const j = i * 12; if (Math.min(lum(col, j), lum(col, j + 4), lum(col, j + 8)) < 175) d++; }
      return d / rows > 0.6;
    };
    const reach = Math.round(W * 0.45);
    for (let x = cx - 2; x > Math.max(0, cx - reach); x--) { if (darkCol(x)) { out.left = (x + 1) / W; break; } }
    for (let x = cx + 2; x < Math.min(W - 1, cx + reach); x++) { if (darkCol(x)) { out.right = x / W; break; } }
    if (out.left == null || out.right == null || out.right - out.left < 0.03) { out.left = null; out.right = null; }
  } catch (e) {}
  return out;
}
function quickTextAt(page, fx, fy) {
  if (!pdfView.hasDoc()) return null;
  // 1) a detected field under the tap → its own element
  const f = lastDet && lastDet.fields && lastDet.fields.find((d) => d.type === 'text' && d.page === page
    && fx >= d.fx - 0.004 && fx <= d.fx + (d.fw || 0) + 0.004
    && fy >= d.fy - (d.fh || 0.02) * 0.6 && fy <= d.fy + (d.fh || 0.02) * 1.6);
  const afterBlur = (ctrl) => {
    const inner = ctrl.node.querySelector('.txt');
    if (inner) inner.addEventListener('blur', () => { setTimeout(() => { try { fieldsPanel.syncValues(panelValueMap()); } catch (e) {} }, 0); }, { once: true });
  };
  if (f) {
    const existing = overlay.getElements().find((c) => c.model.type === 'text' && c.model.fieldKey === f.fieldKey);
    if (existing) { overlay.selectCtrl(existing); existing.focusText(); return existing; }
    const size = Math.max(f.fontFrac || 0, MIN_FILL_FRAC * 0.85) || docUniformSize();
    const align = /[֐-׿؀-ۿ]/.test(f.label || '') ? 'right' : 'left';
    const fh = size * 1.15;
    const ctrl = overlay.addModelAt('text', page, {
      fx: align === 'right' ? f.fx + (f.fw || 0) : f.fx, fy: f.fy + Math.max(0, ((f.fh || fh) - fh) / 2),
      fontFrac: size, align, fieldKey: f.fieldKey, cellW: f.fw || null, cellX: f.fx, text: '', quick: true
    }, true);
    if (ctrl) { afterBlur(ctrl); overlay.setFieldActive && overlay.setFieldActive(f.fieldKey); }
    return ctrl;
  }
  // 2) free text that reads the paper under it
  const size = docUniformSize();
  const ink = inkCellAt(page, fx, fy, size);
  const fh = size * 1.15;
  const extra = { fx, fy: ink.lineY != null ? ink.lineY - fh + fh * 0.12 : fy - fh / 2, fontFrac: size, align: 'right', text: '', quick: true };
  if (ink.left != null) { extra.cellX = ink.left + 0.004; extra.cellW = ink.right - ink.left - 0.008; extra.fx = Math.min(fx, ink.right - 0.004); }
  const ctrl = overlay.addModelAt('text', page, extra, true);
  if (ctrl) afterBlur(ctrl);
  return ctrl;
}
// one-time nudge: the paper itself is the input
function hintClickToType() {
  try {
    if (PFS.store.get('hint_clicktype', false)) return;
    PFS.store.set('hint_clicktype', true);
    PFS.toast('💡 לחיצה על כל מקום בטופס פותחת סמן — פשוט מקלידים. הטקסט מתיישב לבד על הקו ובתוך המשבצת.', 'ok', 6000);
  } catch (e) {}
}
...
async function runDetection(force) {
  ...
  try {
    ...
    if (cached) {
      ...
      try { liftSmallFills('cached-detect'); } catch (e) {}
      try { if (certMode && certAnchorToDetected(true)) { if (certCard && certCard.__render) certCard.__render(); } } catch (e) {}
      PFS.toast(nCa
    ...
    try { liftSmallFills('detect'); } catch (e) {}
    // certificate: the printed labels just found are the exact anchors for
    // the fields that were auto-stacked before detection finished
    try { if (certMode && certAnchorToDetected(true)) { if (certCard && certCard.__render) certCard.__render(); } } catch (e) {}
    if (det.tier === 'scanned') {
    ...
    else if (!(certOpening || certMode)) PFS.toast('לא זוהו שדות אוטומטית', 'err');   // a certificate has none — expected
  } catch (e) {
...
async function openCompanion(link) {
  try {
    ...
    if (extracted['סניף']) {
      const br = PFS.vault.norm(extracted['סניף']);
      const camps = PFS.store.get('org_campuses', []) || [];
      let addr = (camps.find((cp) => PFS.vault.norm(cp.name + ' ' + cp.address).includes(br)) || {}).address;
      if (!addr && PFS.patterns) {
        const opt = PFS.patterns.optionsFor({ label: 'כתובת מוסד ההכשרה', type: 'text' })
...
$('compInput') && $('compInput').addEventListener('change', async (e) => {
  ...
  try {
    const compBytes = await f.arrayBuffer();
    const rec = await PFS.companions.add({
      ownerFp: currentFp, ownerName: currentFileName,
      name: f.name, bytes: compBytes
    });
    cloudPutDoc('comp', rec.id, compBytes.slice(0), { name: rec.name });
    renderCompanions();
...
async function openPdfFile(file) {
  ...
  try {
    ...
    const myGen = loadGen;
    lastDet = null;
    if (!certOpening) { try { closeCertWizard(); } catch (e) {} }   // a plain open leaves certificate mode
    ...
    // history: local first, then the user's private cloud folder (if logged in)
    PFS.recent && PFS.recent.save(file.name, buf.slice(0)).then((id) => {
      renderRecent();
      if (id) cloudPutDoc('recent', id, buf.slice(0), { name: file.name });
    });
    ...
    runDetection();
    if (!certOpening) hintClickToType();
    // first-run guided tour — now that every zone on screen is alive
    if (!certOpening) PFS.tour && PFS.tour.maybeStart();   // the certificate wizard is its own guide
  } catch (e) {
    console.error(e);
...
async function doExport(opts) {
  // The export is WYSIWYG: it bakes EXACTLY what the editor shows. This used
  // to quietly re-run the handwriting uniformizer, which shrank every value
  // back to the detected document size the moment export opened ("כשאני מייצא
  // הוא מקטין מייד את הפונטים"). Sizes are normalized at creation; evening
  // them later is only ever the explicit "כתב אחיד" button.
  if (!pdfView.hasDoc()) return;
  const models = overlay.getElements().map((c) => c.model);
...
vp.addEventListener('drop', async (e) => {
  ...
  if (file) { openPdfFile(file); return; }
  // a student list dropped on the home screen → certificates, one touch
  const list = files.find((f) => /\.(xlsx|csv)$/i.test(f.name));
  if (list && !pdfView.hasDoc()) {
    try {
      const parsed = /\.xlsx$/i.test(list.name)
        ? PFS.merge.parseXlsx(new Uint8Array(await list.arrayBuffer()))
        : PFS.merge.parseCSV(await list.text());
      await certAutoFromList(parsed, list.name);
    } catch (err) { console.error(err); PFS.toast('קריאת הרשימה נכשלה', 'err'); }
  }
});
  ...
  formB.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!hit) {
        ...
        cloudPutDoc('lib', libId, copy.slice(0), { name: currentFileName });
      } else libId = hit.id;
...
/* assembleLineRTL(items) — reconstruct one visual line of a Hebrew PDF whose
 * text layer is per-glyph fragments: sort right-to-left (visual RTL = logical
 * Hebrew), join with a space only across real gaps, and flip maximal runs of
 * digit/latin fragments back to left-to-right (they are LTR within RTL). */
function assembleLineRTL(items) {
  const sorted = items.slice().sort((x, y) => (y.transform[4] - x.transform[4]));
  // un-reverse consecutive LTR fragments (digits, latin, ./-)
  const isLTR = (t) => /^[\d\sA-Za-z./\-:]+$/.test(t) && /[\dA-Za-z]/.test(t);
  const out = [];
  for (let i = 0; i < sorted.length; i++) {
    if (isLTR(sorted[i].str)) {
      let j = i; while (j + 1 < sorted.length && isLTR(sorted[j + 1].str)) j++;
      for (let k = j; k >= i; k--) out.push(sorted[k]);
      i = j;
  ...
  out.forEach((it) => {
    if (prev) {
      ...
      // the EDGE-to-EDGE gap between the two fragments' boxes — origin
      // distance flags every mid-word split as a space
      const aL = it.transform[4], aR = aL + (it.width || 0);
      const bL = prev.transform[4], bR = bL + (prev.width || 0);
      const gap = Math.max(aL - bR, bL - aR, 0);
      if (gap > fh * 0.28 && !line.endsWith(' ')) line += ' ';
    }
    line += it.str;
    prev = it;
  });
...
PFS.assembleLineRTL = assembleLineRTL;
...
async function readDocText() {
  ...
  for (let i = 1; i <= n; i++) {
    ...
    lines.forEach((ln) => { b += assembleLineRTL(ln.items) + '\n'; });
  }
  return { a, b };
}
...
// =====================================================================
//  🎓 Certificates — format → where's the name → Excel list → produce
//  One guided card inside Fillo (nothing external): the certificate format
//  (PDF or image) opens like any document, the clerk clicks where the name
//  goes, loads the class list as it is, and gets one certificate per
//  student — a ZIP of named PDFs or one PDF for the printer. Placements
//  ride the normal per-form memory, so the next cohort is two clicks.
// =====================================================================
let certMode = false, certOpening = false, certCard = null, certList = null, certMap = {};
...
const CERT_FIELDS = [
  ...
  // "שנת לימוד": the CURRENT Hebrew school year, filled by itself (a column
  // in the list still wins when there is one)
  { key: 'שנת לימוד', get sample() { return hebrewAcademicYear(); }, size: 0.024, bold: false, label: 'שנת לימוד' },
  { key: 'ציון', sample: '100', size: 0.028, bold: false, label: 'ציון' },
  { key: 'מספר תעודה', sample: 'מס׳ ' + new Date().getFullYear() + '-0001', size: 0.02, bold: false, label: 'מספר תעודה' }
];
// ---- the Hebrew school year (תשפ״ז) ----
// 5787 → תשפ״ז: thousands dropped, letters by value, gershayim before the
// last letter (a single letter takes a geresh); 15/16 are ט״ו/ט״ז, never י־ה
function hebrewYearLetters(year) {
  let n = year % 1000;
  const out = [];
  [[400, 'ת'], [300, 'ש'], [200, 'ר'], [100, 'ק']].forEach(([v, l]) => { while (n >= v) { out.push(l); n -= v; } });
  if (n === 15) { out.push('ט', 'ו'); n = 0; } else if (n === 16) { out.push('ט', 'ז'); n = 0; }
  for (const [v, l] of [[90, 'צ'], [80, 'פ'], [70, 'ע'], [60, 'ס'], [50, 'נ'], [40, 'מ'], [30, 'ל'], [20, 'כ'], [10, 'י']]) { if (n >= v) { out.push(l); n -= v; break; } }
  if (n) out.push(['', 'א', 'ב', 'ג', 'ד', 'ה', 'ו', 'ז', 'ח', 'ט'][n]);
  if (out.length === 1) return out[0] + '׳';
  return out.slice(0, -1).join('') + '״' + out[out.length - 1];
}
function hebrewYearOf(date) {
  try {
    const s = new Intl.DateTimeFormat('en-u-ca-hebrew', { year: 'numeric' }).format(date);
    const m = /(\d{4})/.exec(s); if (m) return +m[1];
  } catch (e) {}
  const y = date.getFullYear() + 3760;   // fallback: Rosh Hashana ≈ mid-September
  return (date.getMonth() > 8 || (date.getMonth() === 8 && date.getDate() >= 15)) ? y + 1 : y;
}
// the school year runs September → summer: from September the year that
// begins at the coming Tishrei (September 1st 2026 is already תשפ״ז), until
// August the year still running
function hebrewAcademicYear(date) {
  const d = date || new Date();
  const ref = d.getMonth() >= 8 ? new Date(d.getFullYear() + 1, 0, 15) : d;
  return hebrewYearLetters(hebrewYearOf(ref));
}
PFS.hebrewAcademicYear = hebrewAcademicYear; PFS.hebrewYearLetters = hebrewYearLetters;
// ---- the certificate registry: every issued certificate, with its serial ----
// {batches:[{id, at, format, count, entries:[{serial, name, id, course}]}]}
...
function certCommitSerials(count) {
  const y = new Date().getFullYear();
  PFS.store.set('cert_serial_next_' + y, (PFS.store.get('cert_serial_next_' + y, 1) || 1) + count);
}
function certAlreadyIssued(idValue, format) {
  if (!idValue) return null;
  for (const b of certRegistry().batches) {
    if (format && b.format !== format) continue;
    const hit = (b.entries || []).find((e) => e.id === idValue);
    if (hit) return { serial: hit.serial, at: b.at, format: b.format };
  }
  return null;
}
...
async function imageToPdfFile(file) {
  const { PDFDocument } = PFS.PDFLib || window.PDFLib;
  const bytes = new Uint8Array(await file.arrayBuffer());
  const doc = await PDFDocument.create();
  let img;
  if (/png$/i.test(file.type) || /\.png$/i.test(file.name)) img = await doc.embedPng(bytes);
  else {
    // webp/jpeg: normalize through a canvas to JPEG (pdf-lib embeds jpg/png only)
    const url = URL.createObjectURL(file);
    const im = await new Promise((res, rej) => { const i = new Image(); i.onload = () => res(i); i.onerror = rej; i.src = url; });
    const c = document.createElement('canvas'); c.width = im.naturalWidth; c.height = im.naturalHeight;
    c.getContext('2d').drawImage(im, 0, 0);
    URL.revokeObjectURL(url);
    const jpg = await (await fetch(c.toDataURL('image/jpeg', 0.95))).arrayBuffer();
    img = await doc.embedJpg(new Uint8Array(jpg));
  }
  ...
  return new File([await doc.save()], file.name.replace(/\.[^.]+$/, '') + '.pdf', { type: 'application/pdf' });
}
async function startCertFlow(file) {
  if (!file) return;
  let pdf = file;
  if (file.type !== 'application/pdf') {
    try { pdf = await imageToPdfFile(file); } catch (e) { PFS.toast('לא הצלחתי לקרוא את התמונה', 'err'); return; }
  }
  certOpening = true;
...
// sentences carrying {משתנים}
function certPlacedFields() {
  return overlay.getElements().filter((c) => c.model.type === 'text' && String(c.model.text || '').trim()
    && (c.model.fieldKey || /\{[^{}]+\}/.test(c.model.text)));
}
// the variable names in play: field keys + every {token} inside sentences
function certKeys() {
  const keys = [];
  certPlacedFields().forEach((c) => {
    if (c.model.fieldKey && !keys.includes(c.model.fieldKey)) keys.push(c.model.fieldKey);
    PFS.merge.tokensIn(c.model.text).forEach((t) => { if (!keys.includes(t)) keys.push(t); });
  });
  return keys;
}
const CERT_SENTENCE = 'מוענקת בזאת ל־{שם מלא}, ת"ז {תעודת זהות},\nעל סיום בהצלחה של קורס {שם הקורס}';
// a sentence with variables: ONE text block that keeps its natural flow while
// {שם מלא}, {תעודת זהות}… fill per student. Wraps inside the page width.
function certPlaceSentence(pageIndex, fx, fy) {
  ...
  PFS.toast('לחיצה כפולה על המשפט כדי לערוך אותו — השאירו את ה־{משתנים} במקומם', 'ok', 5000);
  return ctrl;
}
// place a sample value CENTRED on the clicked spot — the box centre is what
// the batch keeps for every real name, short or long
function certPlace(pageIndex, fx, fy, key) {
  ...
  ctrl.layout();
  ctrl.model.certAuto = [ctrl.model.fx, ctrl.model.fy];   // "untouched" marker for later anchoring
  overlay.deselectAll();
  markDirty();
...
// not float somewhere near them ("זה חייב להיראות יותר סימטרי ומדויק").
const CERT_LABEL_KEY = [
  ['שנת לימוד', /שנת\s*(ה)?לימוד|שנה["״]ל|שנת\s*לימודים/],
  ['תאריך סיום', /סיום|גמר/], ['תאריך התחלה', /התחלה|תחילת|פתיחה/], ['תאריך הנפקה', /הנפקה|הופק|ניתן/],
  ['ציון', /ציון/], ['מספר תעודה', /מס(פר)?['׳]?\s*(ה)?תעודה|סידורי/]
];
function certKeyForLabel(label) {
  const L = String(label || '').trim();
  if (!L) return null;
  const canon = PFS.vault.matchKey(L);
  if (canon === 'full_name') return 'שם מלא';
  if (canon === 'id') return 'תעודת זהות';
  for (const [k, re] of CERT_LABEL_KEY) if (re.test(L)) return k;
  if (canon === 'course_name' || /קורס|מסלול|תכנית|תוכנית/.test(L)) return 'שם הקורס';
  if (/תאריך|מועד/.test(L)) return 'תאריך סיום';
  return null;
}
// centre a field inside a detected blank, at the blank's own size
function certPlaceInBlank(c, f) {
  const m = c.model;
  if (f.page !== m.page) return false;
  const spec = CERT_FIELDS.find((s) => s.key === m.fieldKey) || { size: 0.03 };
  m.fontFrac = PFS.clamp(f.fh * 0.9, 0.014, spec.size);
  m.align = 'center'; m.wrapW = null; m.cellW = null; m.cellX = null;
  m.maxW = f.fw;
  c.layout();
  m.fx = PFS.clamp(f.fx + f.fw / 2 - m.fw / 2, 0, 1 - m.fw);
  m.fy = f.fy + f.fh / 2 - m.fh / 2;
  m.certAnchor = 'label'; m.certAuto = [m.fx, m.fy];
  c.layout();
  return true;
}
// every placed field whose meaning matches a printed label moves into that
// label's blank. onlyUntouched: leave anything the clerk already dragged.
function certAnchorToDetected(onlyUntouched) {
  if (!lastDet || !lastDet.fields || !lastDet.fields.length) return 0;
  const on = certOnPageMap();
  const used = new Set();
  let moved = 0;
  Object.keys(on).forEach((key) => {
    if (/^__/.test(key)) return;
    const c = on[key], m = c.model;
    if (onlyUntouched && !(m.certAuto && Math.abs(m.fx - m.certAuto[0]) < 1e-4 && Math.abs(m.fy - m.certAuto[1]) < 1e-4)) return;
    const f = lastDet.fields.find((f2) => f2.type === 'text' && !used.has(f2) && (f2.fw || 0) > 0.03 && f2.page === m.page && certKeyForLabel(f2.label) === key);
    if (!f) return;
    used.add(f);
    if (certPlaceInBlank(c, f)) moved++;
  });
  if (moved) { markDirty(); scheduleSnap(); }
  return moved;
}
// a field DROPPED near a printed underline settles onto it: centred on the
// line's own extent, sitting just above it (works on scanned/image formats
// too — it reads the rendered pixels, not a text layer)
function certSnapToInk(c) {
  const m = c.model; if (!m || m.type !== 'text') return false;
  const views = pdfView.viewList ? pdfView.viewList() : [];
  const view = views[m.page], canvas = view && view.canvas;
  if (!canvas || !canvas.width) return false;
  const ctx = canvas.getContext('2d', { willReadFrequently: true });
  const W = canvas.width, H = canvas.height;
  const fh = m.fh || 0.04, fw = m.fw || 0.2;
  const lum = (d, j) => 0.299 * d[j] + 0.587 * d[j + 1] + 0.114 * d[j + 2];
  // look from the middle of the box to a hand's width below it (a small
  // field — an ID at 2.4% — must still find a line dropped 5% under it)
  const yFrom = Math.max(0, Math.round((m.fy + fh * 0.5) * H)), yTo = Math.min(H - 1, Math.round((m.fy + fh + Math.max(fh * 1.5, 0.07)) * H));
  const x0 = Math.max(0, Math.round((m.fx - fw * 0.3) * W)), x1 = Math.min(W, Math.round((m.fx + fw * 1.3) * W));
  if (x1 - x0 < 10 || yTo <= yFrom) return false;
  let lineY = -1;
  ...
  let s = -1;
  for (let d = 0; d < (x1 - x0) / 2 && s < 0; d++) { if (cx + d < W && ink(cx + d)) s = cx + d; else if (cx - d >= 0 && ink(cx - d)) s = cx - d; }
  if (s < 0) return false;
  let L = s, R = s, gap = 0;
  while (L > 0 && gap < 4) { L--; gap = ink(L) ? 0 : gap + 1; } L += gap;
  gap = 0; while (R < W - 1 && gap < 4) { R++; gap = ink(R) ? 0 : gap + 1; } R -= gap;
  const extent = (R - L) / W;
  if (extent > 0.9 || extent < 0.05) return false;   // a page border / a speck
  m.align = 'center'; m.wrapW = null; m.cellW = null; m.cellX = null; m.maxW = extent;
  c.layout();
  m.fx = PFS.clamp((L + R) / 2 / W - m.fw / 2, 0, 1 - m.fw);
  m.fy = lineY / H - m.fh + m.fh * 0.12;   // the glyphs SIT on the line (baseline ≈ line), like a hand would write
  m.certAnchor = 'line'; m.certAuto = [m.fx, m.fy];
  c.layout(); markDirty(); scheduleSnap();
  PFS.toast('📏 יושר על הקו המודפס', 'ok', 1500);
  return true;
}
// one click: every free-floating field shares ONE axis (the page centre when
// they are near it) and the stack gets an even rhythm — top and bottom stay,
// the gaps between become equal. Anchored fields are left where they belong.
function certAlignAll() {
  const els = certPlacedFields().filter((c) => !c.model.certAnchor);
  if (els.length < 2) return 0;
  const cxs = els.map((c) => c.model.fx + (c.model.fw || 0) / 2);
  const sorted = cxs.slice().sort((a, b) => a - b);
  const med = sorted[Math.floor(sorted.length / 2)];
  const axis = Math.abs(med - 0.5) < 0.02 ? 0.5 : med;
  let n = 0;
  const onAxis = els.filter((c, i) => Math.abs(cxs[i] - axis) < 0.05);
  onAxis.forEach((c) => {
    const m = c.model; m.align = 'center'; m.wrapW = null; c.layout();
    const nx = PFS.clamp(axis - m.fw / 2, 0, 1 - m.fw);
    if (Math.abs(nx - m.fx) > 1e-4) { m.fx = nx; n++; }
    c.layout();
  });
  const col = onAxis.slice().sort((a, b) => a.model.fy - b.model.fy);
  if (col.length >= 3) {
    const top = col[0].model.fy, bottom = col[col.length - 1].model.fy + col[col.length - 1].model.fh;
    const totalH = col.reduce((s, c) => s + c.model.fh, 0);
    const gap = (bottom - top - totalH) / (col.length - 1);
    if (gap >= 0) { let y = top; col.forEach((c) => { if (Math.abs(c.model.fy - y) > 1e-4) { c.model.fy = y; n++; } c.layout(); y += c.model.fh + gap; }); }
  }
  onAxis.forEach((c) => { c.model.certAuto = [c.model.fx, c.model.fy]; });
...
// ---- the checklist behind step 1 ----
// what can be on a certificate, in the order it is stacked when auto-placed
let certExtraKeys = [];
function certRowDefs() {
  const base = [
    { key: 'שם מלא', label: 'שם הסטודנט', icon: '👤', def: true },
    { key: 'תעודת זהות', label: 'תעודת זהות', icon: '🪪', def: true },
    { key: 'שם הקורס', label: 'שם הקורס', icon: '📚', def: true },
    { key: '__sentence', label: 'משפט עם משתנים (מוענקת בזאת ל־{שם מלא}…)', icon: '📝', def: false },
    { key: 'תאריך סיום', label: 'תאריך סיום', icon: '📅', def: true },
    { key: 'תאריך התחלה', label: 'תאריך התחלה', icon: '📅', def: false },
    { key: 'תאריך הנפקה', label: 'תאריך הנפקה', icon: '📅', def: false },
    { key: 'שנת לימוד', label: 'שנת לימוד (' + hebrewAcademicYear() + ' — מתמלא לבד)', icon: '🗓️', def: false },
    { key: 'ציון', label: 'ציון', icon: '💯', def: false },
    { key: 'מספר תעודה', label: 'מספר תעודה (סידורי, נרשם ביומן)', icon: '🔢', def: false },
    { key: '__signature', label: 'חתימה (מהמאגר)', icon: '✍️', def: false },
    { key: '__stamp', label: 'חותמת (מהמאגר)', icon: '🔏', def: false }
  ];
  certExtraKeys.forEach((k) => { if (!base.some((b) => b.key === k)) base.push({ key: k, label: k, icon: '🏷️', def: false }); });
  return base;
}
// what is on the page right now, keyed like the checklist
function certOnPageMap() {
  const map = {};
  overlay.getElements().forEach((c) => {
    const m = c.model;
    if (m.type === 'text' && m.fieldKey && !map[m.fieldKey]) map[m.fieldKey] = c;
    if (m.type === 'text' && !m.fieldKey && /\{[^{}]+\}/.test(m.text || '') && !map.__sentence) map.__sentence = c;
    if (m.type === 'image' && m.kind === 'signature' && !map.__signature) map.__signature = c;
    if (m.type === 'image' && m.kind === 'stamp' && !map.__stamp) map.__stamp = c;
  });
  return map;
}
// bring the page in line with the ticked list: add what is missing (stacked,
// centred, top to bottom in list order), remove what was un-ticked
function certApplyChecklist(wanted) {
  const on = certOnPageMap();
  let added = 0, removed = 0;
  // remove un-ticked
  Object.keys(on).forEach((k) => { if (!wanted.includes(k)) { overlay.deleteCtrl(on[k]); removed++; } });
  // stack the missing ones below the lowest existing certificate field
  const existing = certPlacedFields().map((c) => c.model.fy + (c.model.fh || 0.04));
  let y = existing.length ? Math.max(...existing) + 0.03 : 0.42;
  const order = certRowDefs().map((r) => r.key);
  wanted.slice().sort((a, b) => order.indexOf(a) - order.indexOf(b)).forEach((k) => {
    if (on[k]) return;
    if (k === '__signature' || k === '__stamp') { certAddAsset(k === '__signature' ? 'signature' : 'stamp'); added++; return; }
    if (k === '__sentence') { const c = certPlaceSentence(0, 0.5, Math.min(y + 0.03, 0.9)); if (c) { y = c.model.fy + c.model.fh + 0.03; added++; } return; }
    if (k === 'מספר תעודה') { certPlace(0, 0.84, 0.92, k); added++; return; }   // a corner, like a real serial
    const spec = CERT_FIELDS.find((f) => f.key === k) || { size: 0.03 };
    const c = certPlace(0, 0.5, Math.min(y + spec.size * 0.6, 0.9), k);
    if (c) { y = c.model.fy + c.model.fh + 0.025; added++; }
  });
...
// named spots on the page — position expressed in words
function certMoveTo(key, slot) {
  const on = certOnPageMap(); const c = on[key]; if (!c) return;
  const m = c.model, fw = m.fw || 0.2, fh = m.fh || 0.04;
  const name = on['שם מלא'];
  const spots = {
    center: [0.5, 0.45], below_name: [0.5, name && name !== c ? name.model.fy + name.model.fh + 0.04 : 0.55],
    bottom_right: [0.8, 0.86], bottom_center: [0.5, 0.86], bottom_left: [0.2, 0.86], top_right: [0.85, 0.08], top_left: [0.15, 0.08]
  };
  const s = spots[slot]; if (!s) return;
  m.fx = PFS.clamp(s[0] - fw / 2, 0, 1 - fw); m.fy = PFS.clamp(s[1] - fh / 2, 0, 1 - fh);
  if (m.wrapW) m.fx = PFS.clamp(s[0] - m.wrapW / 2, 0, 1 - m.wrapW);
  c.layout(); overlay.selectCtrl(c); PFS.scrollToEl(c.node, 'center'); markDirty();
}
...
function showCertWizard() {
  ...
  if (certPendingList && step === 2) {
    const pend = certPendingList; certPendingList = null;
    certLoadList(pend.parsed, pend.fileName);
    if (Object.values(certMap).some(Boolean)) step = 3;
  } else if (certPendingList && step === 1) {
    PFS.toast('📊 הרשימה מחכה — סמן איפה השם, ואז היא תיטען לבד', 'ok', 4000);
  }
  const render = () => {
    const placed = certPlacedFields();
    const keys = certKeys();
    let body = '';
    if (step === 1) {
      // a CHECKLIST, not a click-on-the-page ritual: tick what the certificate
      // shows, press one button, everything lands in a tidy stack in the
      // middle — then only drag to adjust. Nothing hidden, nothing modal.
      const onPage = certOnPageMap();
      const fixedOk = (k) => !/^__/.test(k) && k !== 'שם מלא' && k !== 'תעודת זהות' && k !== 'מספר תעודה';
      const rows = certRowDefs().map((r, i) => {
        const placed = !!onPage[r.key];
        const id = 'certRow' + i;
        return '<div class="cert-row' + (placed ? ' on' : '') + '">' +
          '<div class="cert-row-main">' +
            '<input type="checkbox" id="' + id + '" data-key="' + escapeHtml(r.key) + '" ' + (placed ? 'checked' : '') + '>' +
            '<label for="' + id + '" class="cert-row-l">' + r.icon + ' ' + escapeHtml(r.label) + '</label>' +
            (placed ? '<button type="button" class="cert-find" data-key="' + escapeHtml(r.key) + '" title="הצג על התעודה">🔍</button>' : '') +
          '</div>' +
          (placed && !/^__(signature|stamp)$/.test(r.key)
            ? '<div class="cert-row-more">' +
                '<select class="cert-where" data-key="' + escapeHtml(r.key) + '" title="איפה על התעודה?">' +
                  '<option value="">📍 איפה? (או גררו)</option>' +
                  '<option value="center">במרכז התעודה</option><option value="below_name">מתחת לשם</option>' +
                  '<option value="bottom_right">שורה תחתונה – ימין</option><option value="bottom_center">שורה תחתונה – מרכז</option>' +
                  '<option value="bottom_left">שורה תחתונה – שמאל</option><option value="top_right">פינה ימנית עליונה</option><option value="top_left">פינה שמאלית עליונה</option>' +
                '</select>' +
                (fixedOk(r.key) ? '<input type="text" class="cert-fixed" data-key="' + escapeHtml(r.key) + '" placeholder="ערך קבוע לכולם (אם אין עמודה באקסל)" value="' + escapeHtml(onPage[r.key].model.text || '') + '">' : '') +
              '</div>'
            : '') +
          '</div>';
      }).join('');
      ...
      body = '<div class="gd-q">רשימת הסטודנטים</div>' +
        (restored && !certList ? '<div class="hint" style="margin:-2px 0 8px">📌 המיקומים מהפעם הקודמת שוחזרו — נשאר רק לטעון את הרשימה.</div>' : '') +
        '<div class="gd-chips"><button type="button" class="gd-chip" id="certLoad">📊 טען Excel / CSV</button>' +
        '<button type="button" class="gd-chip" id="certBack1">📍 הוסף/שנה שדות</button></div>' +
        (certList ? '<div class="hint" id="certListInfo" style="margin-bottom:6px">נמצאו <b>' + n + '</b> סטודנטים · שורה ראשונה: <b>' + escapeHtml(String(Object.values(certList.records[0] || {})[0] || '')) + '</b></div>' +
          '<div id="certMapUI" style="display:flex;flex-direction:column;gap:4px;margin-bottom:8px"></div>' +
          '<div class="hint muted" style="margin-bottom:8px">משתנים למשפט: ' + certList.headers.map((h) => '<code>{' + escapeHtml(h) + '}</code>').join(' ') + '</div>' : '') +
        '<div class="gd-foot"><button type="button" class="btn primary" id="certNext" ' + (certList && Object.values(certMap).some(Boolean) ? '' : 'disabled') + '>המשך לבדיקה והפקה ➜</button></div>';
    } else {
      const pf = certPreflightNow();
    ...
    certCard.innerHTML =
      '<div class="gd-progwrap"><div class="gd-prog" style="width:' + Math.round((step - 1) / 3 * 100 + 10) + '%"></div></div>' +
      '<button type="button" class="gd-x" id="certX" title="סגירה" aria-label="סגירת אשף התעודות">✕</button>' +
      '<div class="gd-count">🎓 הנפקת תעודות · שלב ' + step + ' / 3</div>' + body;
    const $c = (id) => certCard.querySelector('#' + id);
    $c('certX').addEventListener('click', closeCertWizard);
    if (step === 1) {
      // a tick IS the action: the field appears on the certificate at once
      certCard.querySelectorAll('.cert-row input[type=checkbox]').forEach((cb) => cb.addEventListener('change', () => {
        const wanted = [...certCard.querySelectorAll('.cert-row input[type=checkbox]')].filter((i) => i.checked).map((i) => i.dataset.key);
        const n = certApplyChecklist(wanted);
        render();
        if (n.added) PFS.toast('✓ הונח על התעודה — גררו למקום הנכון', 'ok', 3000);
      }));
      certCard.querySelectorAll('.cert-find').forEach((b) => b.addEventListener('click', (e) => {
        e.preventDefault(); e.stopPropagation();
        const c = certOnPageMap()[b.dataset.key];
        if (c) { overlay.selectCtrl(c); PFS.scrollToEl(c.node, 'center'); }
      }));
      // "where?" in words — for anyone who would rather not drag on a touchpad
      certCard.querySelectorAll('.cert-where').forEach((sel) => sel.addEventListener('change', () => {
        if (sel.value) { certMoveTo(sel.dataset.key, sel.value); sel.value = ''; }
      }));
      // a value that is the same for everyone, typed once here
      certCard.querySelectorAll('.cert-fixed').forEach((inp) => inp.addEventListener('input', () => {
      ...
      $c('certAddOther').addEventListener('click', async () => {
        ...
        certExtraKeys.push(String(name).trim());
        certApplyChecklist([...certKeys(), String(name).trim()]);
        render();
      });
      $c('certNext').addEventListener('click', () => {
        overlay.setPlacing(null); step = 2;
        if (certPendingList) { const pend = certPendingList; certPendingList = null; certLoadList(pend.parsed, pend.fileName); if (Object.values(certMap).some(Boolean)) step = 3; }
        render();
      });
    } else if (step === 2) {
      $c('certLoad').addEventListener('click', () => $('certListInput').click());
      $c('certBack1').addEventListener('click', () => { step = 1; render(); });
      if (certList) {
        const ui = $c('certMapUI');
        keys.forEach((k) => {
          const row = document.createElement('div'); row.style.cssText = 'display:flex;align-items:center;gap:8px;font-size:12.5px';
          const lab = document.createElement('span'); lab.style.cssText = 'min-width:90px;font-weight:700'; lab.textContent = k;
          const sel = document.createElement('select'); sel.style.cssText = 'flex:1;padding:4px 6px;font-size:12px;border:1px solid var(--line);border-radius:7px;background:var(--surface)';
          const none = document.createElement('option'); none.value = ''; none.textContent = '(השאר את הדוגמה)'; sel.appendChild(none);
          certList.headers.forEach((h) => { const o = document.createElement('option'); o.value = h; o.textContent = h; sel.appendChild(o); });
          sel.value = certMap[k] || '';
          sel.addEventListener('change', () => { certMap[k] = sel.value || null; $c('certNext').disabled = !Object.values(certMap).some(Boolean); });
          row.append(lab, sel); ui.appendChild(row);
        });
      }
  ...
  render();
  certCard.__render = render;
}
// (the old "arm a chip, then click on the page" ritual is gone: non-technical
// clerks never guessed the second step — the checklist above replaced it)
...
// may use any {column}) + the placed keys mapped onto them + the file name
function certBuildRecords() {
  const keys = certKeys();
  const mapping = {};   // header → key (the merge engine's shape)
  keys.forEach((k) => { if (certMap[k]) mapping[certMap[k]] = k; });
  const nameHeader = certMap['שם מלא'] || certMap[keys[0]] || null;
  const mapped = PFS.merge.remapRecords(certList.records, mapping);
  // {שנת לימוד} in a sentence resolves to the current school year unless the
  // list has its own column (a placed FIELD keeps its own, editable text)
  const yearDefault = (keys.includes('שנת לימוד') && !certMap['שנת לימוד'] && !certOnPageMap()['שנת לימוד']) ? { 'שנת לימוד': hebrewAcademicYear() } : {};
...
function certPreflightNow() {
  ...
  if (idHeader) {
    ...
    pf.clean.forEach((r, i) => {
      const prev = certAlreadyIssued(String(r[idHeader] || '').trim(), format);
      if (prev) pf.issues.push({ row: i + 2, kind: 'already_issued', msg: (r.__name || '') + ' כבר קיבל/ה תעודה מהפורמט הזה (' + prev.serial + ', ' + new Date(prev.at).toLocaleDateString('he-IL') + ')' });
    });
  }
  return pf;
}
// three thumbnails before anything is produced: the first row, the longest
// name (the stress case for the layout) and the last row
async function certRenderPreviews(host) {
  if (!host || !certList) return;
  const { clean } = certPreflightNow();
  ...
  for (const rec of picks) {
    try {
      ...
      const img = document.createElement('img'); img.src = cv.toDataURL('image/png'); img.alt = rec.__name || '';
      img.title = 'לחיצה להגדלה'; img.addEventListener('click', () => window.open(img.src, '_blank'));
      const cap = document.createElement('figcaption'); cap.textContent = rec.__name || '—';
      fig.append(img, cap); host.appendChild(fig);
      try { doc.destroy(); } catch (e) {}
    } catch (e) { console.warn('preview failed', e); }
  }
  if (!host.children.length) host.innerHTML = '<span class="hint muted">התצוגה המקדימה נכשלה — אפשר להפיק בכל זאת</span>';
}
async function certProduce(kind, opts) {
  if (!certList) return null;
  const built = certBuildRecords();
  const nameHeader = built.nameHeader;
  // never print a blank certificate: rows without a name are skipped
  const records = PFS.merge.preflight(built.records, { nameKey: nameHeader || '__name', idKey: built.idHeader }).clean;
...
// ONE touch: a student list dropped on the home screen. The last format used
// (or the only one on the shelf) opens by itself, the list loads, and the
// wizard lands on the check-and-preview step — the clerk only approves.
let certPendingList = null;
async function certAutoFromList(parsed, fileName) {
  if (!parsed || !parsed.records.length) { PFS.toast('לא נמצאו שורות ברשימה', 'err'); return false; }
  certPendingList = { parsed, fileName };
  const certs = (await PFS.library.list()).filter((d) => d.kind === 'cert');
  const last = PFS.store.get('cert_last_format', '');
  const pick = certs.find((d) => d.name === last) || (certs.length === 1 ? certs[0] : null);
  if (!pick) {
    showCertIntro();
    PFS.toast(certs.length ? 'בחר את פורמט התעודה — הרשימה תיטען לבד אחריו' : 'בחר קודם פורמט תעודה; הרשימה שגררת תיטען לבד אחריו', 'ok', 5000);
    return false;
  }
  await certOpenFromShelf(pick.id);
  return true;
}
...
$('certListInput') && $('certListInput').addEventListener('change', async (e) => {
  const f = e.target.files[0]; e.target.value = '';
  if (!f) return;
  try {
    const parsed = /\.xlsx$/i.test(f.name)
      ? PFS.merge.parseXlsx(new Uint8Array(await f.arrayBuffer()))
      : PFS.merge.parseCSV(await f.text());
    certLoadList(parsed, f.name);
  } catch (err) {
    console.error(err); PFS.toast('קריאת הרשימה נכשלה — נסו לשמור כ-CSV ולטעון שוב', 'err', 6000);
  }
});
...
function acctStatus(t, kind) {
  document.querySelectorAll('.acct-status').forEach((el) => { el.textContent = t || ''; el.style.color = kind === 'err' ? 'var(--warn)' : (kind === 'ok' ? 'var(--ok,#1a7f4b)' : 'var(--ink-3)'); });
}
...
// ---- the user's documents in their private cloud folder --------------------
// Every document that matters rides along: history (recent), the certificate
// shelf / library (lib), linked appendices (comp). Uploads are fire-and-forget
// (local always wins first); downloads happen on login (library, appendices)
// or on click (history).
function cloudPutDoc(kind, id, bytes, meta) {
  try {
    if (!ACCT().authed() || !bytes || !id) return;
    ACCT().putFile(kind, id, bytes, meta || {}).then(() => acctStatus('נשמר בענן ✓ ' + new Date().toLocaleTimeString('he-IL'), 'ok'))
      .catch((e) => { console.warn('cloud put failed', e); acctStatus('שמירת קובץ בענן נכשלה', 'err'); });
  } catch (e) {}
}
async function cloudHydrate() {
  if (!ACCT().authed()) return;
  try {
    const lib = await PFS.library.list(); const have = new Set(lib.map((d) => d.id));
    ...
    for (const f of ACCT().fileIndex('comp')) {
      ...
      const bytes = await ACCT().getFile('comp', f.id); if (bytes) await PFS.companions.restoreBytes(f.id, bytes.buffer);
    }
  } catch (e) { console.warn('cloud hydrate failed', e); }
  renderRecent(); try { renderLibrary(); } catch (e) {}
}
// a shared computer: the next person must not see the previous one's data
async function wipeLocalForUserSwitch() {
  try { closeCertWizard(); } catch (e) {}
  if (pdfView.hasDoc()) { try { await goHomeSilently(); } catch (e) {} }
  PFS.store.clear(['acct:cfg', 'acct:session', 'acct:last_user', 'sync:cfg', 'onboarded', 'tour_done', 'export_quality']);
  try { await PFS.recent.clearAll(); } catch (e) {}
  try { await PFS.library.clearAll(); } catch (e) {}
  try { await PFS.companions.clearAll(); } catch (e) {}
  refreshAllFromStore(); renderRecent(); try { renderLibrary(); } catch (e) {} try { renderCourses(); } catch (e) {}
}
async function goHomeSilently() {
  const c = PFS.ui.confirm; PFS.ui.confirm = async () => true;
  try { await goHome(); } finally { PFS.ui.confirm = c; }
}
// cloud copies follow local deletions
(function () {
  const wrapDel = (mod, fnName, kind) => {
    const orig = mod[fnName].bind(mod);
    mod[fnName] = async function (id) { const r = await orig(id); try { if (ACCT().authed()) await ACCT().deleteFile(kind, id); } catch (e) {} return r; };
  };
  if (PFS.library) wrapDel(PFS.library, 'remove', 'lib');
  if (PFS.companions) wrapDel(PFS.companions, 'remove', 'comp');
})();
// the login gate: with requireLogin baked in, Fillo opens on the sign-in
// screen and nothing else is usable until a person is signed in
function loginRequired() { const b = window.PFS_SUPABASE || {}; return !!(b.requireLogin && ACCT().configured()); }
function showLoginGate() {
  const m = $('loginModal'); if (!m) return;
  renderAccount($('loginBody'));
  m.classList.add('show');
}
function hideLoginGate() { const m = $('loginModal'); if (m) m.classList.remove('show'); }
function renderAccount(target) {
  const body = target || $('acctBody'); if (!body) return;
  if (!ACCT().configured()) {
  ...
  const q = (id) => body.querySelector('#' + id);   // scoped: the same form renders in the settings card AND the login gate
  if (ACCT().authed()) {
    const u = ACCT().user() || {};
    body.innerHTML = '<div class="hint">מחובר/ת כ־<b dir="ltr">' + (u.email || '—') + '</b></div>' +
      '<div class="hint muted">הכול נשמר בענן אוטומטית — פרטים, טפסים, היסטוריה — וזמין בכל מחשב שתתחברו בו.</div>' +
  ...
  body.innerHTML = '<div class="field"><label>אימייל</label><input id="acctEmail" type="email" dir="ltr" placeholder="you@email.com" autocomplete="username" /></div>' +
    '<div class="field"><label>סיסמה</label><input id="acctPass" type="password" dir="ltr" placeholder="לפחות 6 תווים" autocomplete="current-password" /></div>' +
    ...
    '<div class="hint acct-status" style="margin-top:6px"></div>';
  const email = () => q('acctEmail').value.trim(), pass = () => q('acctPass').value;
  q('acctIn').addEventListener('click', async () => {
    if (acctBusy) return; acctBusy = true; acctStatus('מתחבר…');
    try { await ACCT().signIn(email(), pass()); await acctAfterAuth(); }
    catch (e) { acctStatus((e.message === 'BAD_CREDENTIALS') ? 'אימייל או סיסמה שגויים' : 'ההתחברות נכשלה', 'err'); }
    finally { acctBusy = false; }
  });
  q('acctPass').addEventListener('keydown', (e) => { if (e.key === 'Enter') q('acctIn').click(); });
  q('acctUp').addEventListener('click', async () => {
    if (acctBusy) return; acctBusy = true; acctStatus('יוצר חשבון…');
    try { const r = await ACCT().signUp(email(), pass()); if (r.needsConfirm) { acctStatus('נשלח אליך מייל אימות — אשרו ואז התחברו.', 'ok'); } else { await acctAfterAuth(); } }
    catch (e) { acctStatus(/already/i.test(e.message) ? 'האימייל כבר רשום — התחברו' : ('ההרשמה נכשלה: ' + e.message), 'err'); }
    finally { acctBusy = false; }
  });
}
// after sign-in: a DIFFERENT person than the last one on this computer gets
// a clean slate first (never inherit someone else's forms or history), then
// the account's vault is loaded and the documents hydrated
async function acctAfterAuth() {
  acctStatus('טוען את הנתונים שלך…');
  const uid = (ACCT().user() || {}).id || '';
  const last = PFS.store.get('acct:last_user', '');
  if (last && last !== uid) await wipeLocalForUserSwitch();
  PFS.store.set('acct:last_user', uid);
...
async function acctSignOut() {
  ...
  renderAccount();
  PFS.toast('התנתקת — המחשב הזה נקי מהנתונים שלך', 'ok');
  if (loginRequired()) showLoginGate();
}
async function acctAutoLoadOnStart() {
  if (!ACCT().configured()) { renderAccount(); return; }
...
$('libInput') && $('libInput').addEventListener('change', async (e) => {
  ...
  if (f.type !== 'application/pdf') { PFS.toast('בחר קובץ PDF', 'err'); return; }
  try {
    const libBytes = await f.arrayBuffer();
    const rec = await PFS.library.add(f.name, libBytes);
    cloudPutDoc('lib', rec.id, libBytes.slice(0), { name: rec.name });
    renderLibrary();
...
$('libAddOpenBtn') && $('libAddOpenBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    const libBytes2 = pdfView.getBytes().slice(0);
    const rec = await PFS.library.add(name, libBytes2);
    cloudPutDoc('lib', rec.id, libBytes2.slice(0), { name: rec.name });
    renderLibrary();
...
// Document HISTORY on the home screen: every opened PDF is kept (bytes and
// all, capped at 30) and reopens with one click — auto-memory then restores
// everything that was filled. Collapsed: the 5 freshest. Expanded: the full
// history with search and per-item delete.
let recentExpanded = false;
function relDate(ts) {
  const d = new Date(ts), now = new Date();
  const days = Math.floor((now - d) / 86400000);
  if (d.toDateString() === now.toDateString()) return 'היום ' + d.toLocaleTimeString('he-IL', { hour: '2-digit', minute: '2-digit' });
  if (days < 2 && new Date(now - 86400000).toDateString() === d.toDateString()) return 'אתמול';
  return d.toLocaleDateString('he-IL');
}
async function renderRecent(filter) {
  if (!PFS.recent) return;
  const wrap = $('recentWrap'), list = $('recentList');
  if (!wrap || !list) return;
  let docs = await PFS.recent.list();
  // logged in: the history of every computer this account used — cloud-only
  // entries appear alongside the local ones and download on click
  try {
  ...
  wrap.style.display = '';
  list.innerHTML = '';
  const q = PFS.vault ? PFS.vault.norm(filter || '') : String(filter || '').trim();
  const pool = q ? docs.filter((d) => (PFS.vault ? PFS.vault.norm(d.name) : d.name).includes(q)) : docs;
  const shown = recentExpanded ? pool : pool.slice(0, 5);
  // search box — worth its pixels only once the history is real
  if (recentExpanded && docs.length > 8) {
    const s = document.createElement('input');
    s.type = 'search'; s.id = 'recentSearch'; s.placeholder = '🔎 חיפוש בהיסטוריה…'; s.value = filter || '';
    s.style.cssText = 'width:100%;margin-bottom:6px;padding:7px 10px;border:1px solid var(--line);border-radius:8px;font-size:12.5px';
    s.addEventListener('input', () => renderRecent(s.value).then(() => { const el = $('recentSearch'); if (el) { el.focus(); el.setSelectionRange(el.value.length, el.value.length); } }));
    list.appendChild(s);
  }
  shown.forEach((d) => {
  ...
  if (q && !pool.length) {
    const none = document.createElement('div'); none.className = 'hint muted'; none.textContent = 'אין תוצאות ל"' + filter + '"';
