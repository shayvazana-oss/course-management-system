...
(function (root) {
  ...
  // Is this page "document-like" (mostly flat white with sharp black text/lines)
  // rather than photographic? Such pages get PNG in the secure export so Hebrew
  // glyphs stay crisp instead of picking up JPEG ringing; photos get JPEG. We
  // sample a sparse pixel grid and measure the fraction that are near-neutral
  // extremes (near-white or near-black, low chroma) — high on forms, low on
  // photos. Returns true → prefer PNG.
  function isDocumentLikeCanvas(canvas) {
    try {
      const ctx = canvas.getContext('2d');
      const W = canvas.width, H = canvas.height;
      if (!W || !H) return true;
      const data = ctx.getImageData(0, 0, W, H).data;
      const stride = Math.max(1, Math.floor((W * H) / 40000)); // ~40k samples max
      let mono = 0, total = 0;
      for (let p = 0; p < W * H; p += stride) {
        ...
        total++;
        if ((mx - mn) < 24 && (mx > 230 || mx < 70)) mono++;   // neutral extreme
      }
      return total ? (mono / total) > 0.9 : true;
    } catch (e) { return false; }   // tainted/large → play safe with JPEG
  }
  const JPEG_Q = { draft: 0.82, standard: 0.9, high: 0.95 };
   ...
   */
  async function exportFlattenedPdf({ order, models, attachments, renderBase, onProgress, quality }) {
    ...
    const jq = JPEG_Q[quality] || JPEG_Q.standard;
    // draft keeps everything JPEG for the smallest file; otherwise text-heavy
    // pages go lossless PNG (no ringing around Hebrew glyphs), photos stay JPEG
    const forceJpeg = (quality === 'draft');
    let done = 0;
    for (const idx of idxs) {
      ...
      if (!forceJpeg && isDocumentLikeCanvas(canvas)) {
        ...
        img = await out.embedJpg(canvas.toDataURL('image/jpeg', jq));      // small photo
      }
      out.addPage([wPt, hPt]).drawImage(img, { x: 0, y: 0, width: wPt, height: hPt });
      canvas.width = 0; canvas.height = 0;
