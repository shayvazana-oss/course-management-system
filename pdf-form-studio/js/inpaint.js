...
(function (root) {
  ...
  function patch(src, x, y, w, h, opts) {
    ...
    // row (inkLum now measures distance from the background tone)
    const rowMask = new Uint8Array(h);
    for (let j = 0; j < h; j++) rowMask[j] = (L.inkLum[j] > tol && R.inkLum[j] > tol) ? 1 : 0;
    ...
    for (let i = 0; i < w; i++) colMask[i] = (T.inkLum[i] > tol && B.inkLum[i] > tol) ? 1 : 0;
